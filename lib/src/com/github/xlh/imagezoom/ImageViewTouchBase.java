...
public class ImageViewTouchBase extends ImageView implements IDisposable {
	...
	private OnImageScaleListener mScaleListener;
	 ...
	 * @param rotation
	 */
	protected void setImageBitmap(final Bitmap bitmap, int rotation) {
		...
		if (mListener != null) {
			mListener.onBitmapChanged(mBitmapDisplayed.getBitmap());
		}
	}
	...
	protected void onZoom(float scale) {
		if (mScaleListener != null) {
			mScaleListener.onImageScale(this, scale > 1);
		}
	}
	...
	protected void panBy(float dx, float dy) {
		...
		if (mScaleListener != null) {
			if (dx > srect.left) {
				mScaleListener.onImageLeft();
			} else if (dx < srect.left) {
				mScaleListener.onImageRight();
			}
		}
	...
	public interface OnImageScaleListener {
		...
		abstract void onImageScale(View view, boolean isBiggerThanParent);

		abstract void onImageSingleTap();

		abstract void onImageLeft();

		abstract void onImageRight();
	}
}
