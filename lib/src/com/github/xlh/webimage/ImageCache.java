...
public class ImageCache {
	...
	public Uri saveImageTo(Context context, String data, String filename) {
		final String key = hashKeyForDisk(data);
		synchronized (mDiskCacheLock) {
			while (mDiskCacheStarting) {
				try {
			...
			if (mDiskLruCache != null) {
				...
				try {
					...
					if (snapshot != null) {
						inputStream = snapshot.getInputStream(DISK_CACHE_INDEX);
						if (inputStream != null) {
							return Utils.saveImage(context, inputStream,
									filename);
						}
					}
				} catch (final IOException e) {
	...
	public long getCacheSize() {
		if (mDiskLruCache != null) {
			return mDiskLruCache.getCacheSizeInBytes();
		}
		return 0l;
	}
	...
	public static File getDiskCacheDir(Context context, String uniqueName) {
		...
		final String cachePath = (Environment.MEDIA_MOUNTED.equals(Environment
				...
				&& (getExternalCacheDir(context) != null) ? getExternalCacheDir(
				context).getPath()
