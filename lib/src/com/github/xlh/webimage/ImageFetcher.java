...
public class ImageFetcher extends ImageResizer {
			...
			if (ImageCache.getUsableSpace(mHttpCacheDir) > HTTP_CACHE_SIZE) {
				try {
					mHttpDiskCache = DiskLruCache.open(mHttpCacheDir, 1, 1,
							HTTP_CACHE_SIZE);
					XLog.d(TAG, "HTTP cache initialized");
				} catch (IOException e) {
					mHttpDiskCache = null;
				}
			}
			...
			if (mHttpDiskCache != null) {
				try {
					if (!mHttpDiskCache.isClosed()) {
						...
						mHttpDiskCache = null;
						XLog.d(TAG, "HTTP cache closed");
					}
				} catch (IOException e) {
					XLog.e(TAG, "closeCacheInternal - " + e);
				}
			}
		}
	}
	...
	public Uri saveImage(Context context, String url) {
		...
		synchronized (mHttpDiskCacheLock) {
			...
			if (mHttpDiskCache != null) {
				try {
					snapshot = mHttpDiskCache.get(key);
					if (snapshot == null) {
						return mImageCache.saveImageTo(context, url, filename);
					} else {
						fileInputStream = (FileInputStream) snapshot
								.getInputStream(DISK_CACHE_INDEX);
						return Utils.saveImage(context, fileInputStream,
								filename);
					}
				} catch (IOException e) {
					e.printStackTrace();
